...
void ScaleSetToZero(void)
{
	...
	Scale_Data.ScaleDataZero = Scale_Data_Temp / 100;
}
...
void UpdateScaleData(void)
{
	...
	ScaleDataAverage = Scale_Data_Temp / 5.0;
...
//	Scale_Data.ScaleData = ((int32_t)ScaleDataAverage - (int32_t)Scale_Data.ScaleDataZero);
	...
	{
		Scale_Data.ScaleData = (int32_t)(((float)ScaleDataAverage - Scale_Data.ScaleDataZero)/-93.0+0.5);
	}
	else
	{
		Scale_Data.ScaleData = (int32_t)(((float)ScaleDataAverage - Scale_Data.ScaleDataZero)/-93.0-0.5);
